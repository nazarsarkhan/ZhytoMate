        ...
        {filteredConversations.map((conversation) => (
          <Link key={conversation.id} className="motion-card interactive-card flex w-full items-start gap-3 rounded-2xl bg-surface p-4 text-left shadow-sm transition active:scale-[0.98] md:p-5" to={`/chat-history/${conversation.id}`}>
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="mt-1 block text-sm text-on-surface-variant">{formatDate(conversation.lastMessageAt, i18n.resolvedLanguage)}</span>
            </span>
            <Icon name="chevron_right" className="mt-2 text-outline" />
          </Link>
        ))}
        {!conversations.isLoading && !filteredConversations.length ? <p className="rounded-xl border border-outline-variant/30 bg-surface-container-lowest p-6 text-center text-sm text-on-surface-variant">{t("chat.empty")}</p> : null}
      </main>
