...
export default function NewsPage() {
  ...
  const categories = useMemo(
    () => ["all", "utilities", "transport", "official", "events"].map((value) => ({ value, label: categoryLabels[value] })),
    [],
  );
    ...
    <Shell className="bg-background pb-28">
      ...
      <main className="mx-auto grid w-full max-w-6xl gap-3 px-container-padding py-section-margin sm:px-6 md:grid-cols-2 md:px-8 lg:grid-cols-3">
        {filteredNews.map((item) => (
          <Link key={item.id} className="motion-card interactive-card flex gap-4 rounded-xl border border-surface-variant bg-surface p-4 text-left shadow-soft transition active:scale-[0.98]" to={`/news/${item.id}`}>
        ...
        {!filteredNews.length ? <p className="col-span-full rounded-xl border border-outline-variant/30 bg-surface-container-lowest p-6 text-center text-sm text-on-surface-variant">Новини не знайдено</p> : null}
      </main>
