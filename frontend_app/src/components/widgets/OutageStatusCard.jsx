...
function CardFrame({ to, label, icon, tone, title, subtitle }) {
  return (
    <Link to={to} className={CARD_CLASS}>
      <span className="mb-3 block text-xs font-medium leading-snug text-on-surface-variant">{label}</span>
      <div className="flex min-h-0 flex-1 items-start gap-3 md:flex-col md:justify-end">
        ...
        <div className="min-w-0 flex-1 md:w-full md:flex-none">
          <h2 className="max-w-full break-words text-xl font-bold leading-tight text-on-surface sm:text-2xl md:text-xl">{title}</h2>
          <p className={`mt-1 max-w-full break-words text-xs font-medium leading-snug ${tone}`}>{subtitle}</p>
        </div>
      </div>
    </Link>
  );
