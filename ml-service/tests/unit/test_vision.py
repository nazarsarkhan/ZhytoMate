...
Purpose:   Unit: VisionService._parse_and_validate (enum/severity guard, None on any error) and
           ...
           VisionService with placeholder deps — no async calls, no real Gemini.
Layer:     test
May import:   pytest, app.services.vision_service, app.schemas.vision, stdlib
Must NOT import:  app.components, asyncpg, google-genai (pure/fast unit test — vision_service defers the SDK import)
"""
...
def _service() -> VisionService:
    ...
    return VisionService(gemini=object(), settings=object())
